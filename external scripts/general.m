...
if script.BoldAxesCheckBox.Value == true
    ax.FontWeight = 'bold';
end

% Set axes labels
xlabel(ax,script.XAxisEditField.Value);
ylabel(ax,script.YAxisEditField.Value);

% Set axes limits
ax.XLim = [script.LowerXEditField.Value script.UpperXEditField.Value];
ax.YLim = [script.LowerYEditField.Value script.UpperYEditField.Value];

% Set axes scale
ax.XScale = script.XScaleDropDown.Value;
ax.YScale = script.YScaleDropDown.Value;

% Set axes orientation
value = string(script.InvertY.Value);
tf = strcmp(value,'On');
if tf == 1
    set(ax, 'YDir', 'reverse');
else
...
value = string(script.InvertX.Value);
tf = strcmp(value,'On');
if tf == 1
    set(ax, 'XDir', 'reverse');
else
...
% Set data format
if script.XAxisSciNotationCheckBox.Value == true
    ax.XRuler.Exponent = 1;
else
    ax.XRuler.Exponent = 0;
end

if script.YAxisSciNotationCheckBox.Value == true
...
if isempty(findobj(fig,'Type','Line')) == 0
    ...
    if script.ColoursDropDown.Value == "Multiple Data Sets"
    ...
    for count = 1:1:NumberOfLines
        ...
        plotLineData(count).LineWidth = script.LineWidthEditField.Value;
        plotLineData(count).Color = ColorPallet(count,:);
        legend(split(script.LegendEditField.Value, ";"),'Location','best','fontsize',12) %Change legend entries in the curly brackets as needed
    end
    legend('boxoff');
end
...
if isempty(findobj(fig,'Type','Scatter')) == 0
    ...
    for count = 1:1:NumberOfDataSets
        ...
        plotScatterData(count).SizeData = script.MarkerSizeEditField.Value;
...
% fig.WindowStyle = 'normal';
value = script.FigPresetDropDown.Value;
